...
//! Shows active alarms, quick-timer presets, and a dismiss/clear interface.
...
pub fn alarm_panel(manager: &AlarmManager) -> Element<'_, Message> {
...
/// A single alarm row: label, remaining time, kind badge, and delete button.
fn alarm_row<'a>(alarm: &'a Alarm) -> Element<'a, Message> {
    ...
    let info_col = column![label, row![remaining, kind].spacing(6)].spacing(1);

    container(row![info_col, delete_btn].spacing(6))
        .padding(Padding::from([3, 6]))
